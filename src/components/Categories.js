...
// ── Image Editor Modal (free aspect ratio) ──────────────────────────────────
function ImageEditor({ file, onConfirm, onCancel }) {
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      setImg(image);
      const w = Math.min(image.width, image.width);
  ...
  useEffect(() => {
    ...
    // Scale preview so largest dimension = PREVIEW
    ...
    ctx.drawImage(img, offset.x, offset.y, cropW, cropH, 0, 0, pw, ph);
    // grid
    ctx.strokeStyle = 'rgba(255,255,255,0.12)';
    ctx.lineWidth = 1;
    [1, 2].forEach(i => {
      ctx.beginPath(); ctx.moveTo((pw / 3) * i, 0); ctx.lineTo((pw / 3) * i, ph); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0, (ph / 3) * i); ctx.lineTo(pw, (ph / 3) * i); ctx.stroke();
    });
  ...
  const handleMouseMove = (e) => {
    ...
    const ox = dragStart.ox + (dragStart.mx - e.clientX) * scale;
    const oy = dragStart.oy + (dragStart.my - e.clientY) * scale;
  ...
  const handleWidthChange = (val) => {
    ...
    if (lockRatio && cropH > 0) {
      ...
      const h = Math.max(50, Math.min(img.height, Math.round(w * ratio)));
      setCropH(h);
      setOffset(prev => clamp(prev.x, prev.y, w, h, img));
    } else {
      setOffset(prev => clamp(prev.x, prev.y, w, cropH, img));
    }
    setCropW(w);
  };
  ...
  const handleHeightChange = (val) => {
    ...
    if (lockRatio && cropW > 0) {
      ...
      const w = Math.max(50, Math.min(img.width, Math.round(h * ratio)));
      setCropW(w);
      setOffset(prev => clamp(prev.x, prev.y, w, h, img));
    } else {
      setOffset(prev => clamp(prev.x, prev.y, cropW, h, img));
    }
    setCropH(h);
  };
  ...
  const handleConfirm = () => {
    ...
    ctx.drawImage(img, offset.x, offset.y, cropW, cropH, 0, 0, cropW, cropH);
    out.toBlob(blob => {
  ...
  return (
    <div style={s.editorOverlay}>
      <div style={s.editorBox}>
        <h3 style={s.editorTitle}>🖼️ Crop &amp; Resize Image</h3>
        <p style={s.editorHint}>Drag to reposition · Adjust width &amp; height freely</p>

        {img ? (
          <>
            <canvas
              ref={canvasRef}
              style={{ ...s.previewCanvas, cursor: dragging ? 'grabbing' : 'grab' }}
            ...
            <div style={s.dimGrid}>
              ...
              <div style={s.dimBlock}>
                <div style={s.dimLabelRow}>
                  ...
                  <span style={s.dimVal}>{cropW}px</span>
                </div>
                <input type="range" min={50} max={maxW} value={cropW}
                  onChange={e => handleWidthChange(Number(e.target.value))} style={s.slider} />
              </div>
              ...
              <button
                ...
                style={{ ...s.lockBtn, background: lockRatio ? '#4f46e5' : '#e5e7eb' }}
              >
                {lockRatio ? '🔒' : '🔓'}
              </button>
              ...
              <div style={s.dimBlock}>
                <div style={s.dimLabelRow}>
                  <span style={s.dimLabel}>Height</span>
                  <span style={s.dimVal}>{cropH}px</span>
                </div>
                <input type="range" min={50} max={maxH} value={cropH}
                  onChange={e => handleHeightChange(Number(e.target.value))} style={s.slider} />
              </div>
            </div>
            ...
            <div style={s.presetRow}>
              {[
                ...
                <button key={label} type="button" style={s.presetBtn} onClick={() => {
                  if (!img) return;
            ...
            <div style={s.editorMeta}>
              Original: {img.width}×{img.height} · Crop: {cropW}×{cropH} · Output: {cropW}×{cropH}
            </div>
          </>
        ) : (
          <div style={s.editorLoading}>Loading image…</div>
        )}

        <div style={s.editorActions}>
          <button style={s.btnCancel} onClick={onCancel}>Cancel</button>
          <button style={s.btnConfirm} onClick={handleConfirm} disabled={!img}>Use this crop</button>
        </div>
      </div>
    </div>
  );
...
function Categories() {
  ...
  useEffect(() => {
    ...
    const handlePaste = (e) => {
      ...
      if (imgItem) openEditor(imgItem.getAsFile());
    };
    window.addEventListener('paste', handlePaste);
    return () => window.removeEventListener('paste', handlePaste);
  }, [showModal]);

  const openEditor  = (file) => { setRawFile(file); setShowEditor(true); };
  const handleFileChange = (e) => { if (e.target.files[0]) openEditor(e.target.files[0]); };
  ...
  const removeImage = () => { setCroppedFile(null); setCroppedPreview(''); };
  ...
  const handleSubmit = async (e) => {
    ...
    if (croppedFile) form.append('image', croppedFile);

    try {
      if (editingCat) {
        await api.put(`/admin/categories/${editingCat.id}`, form, { headers: { 'Content-Type': 'multipart/form-data' } });
        alert('Category updated');
      } else {
        await api.post('/admin/categories', form, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const closeModal = () => {
    ...
    setCroppedFile(null); setCroppedPreview(''); setRawFile(null);
  };
        ...
        <div className="modal" onClick={closeModal}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              <input
                type="text" placeholder="Category name" value={formData.name} required
              ...
              >
                {croppedPreview ? (
                  <div style={s.previewWrapper}>
                    <img src={croppedPreview} alt="preview" style={s.thumbPreview} />
                    <div style={s.previewMeta}>
                      <span style={s.previewName}>{croppedFile?.name}</span>
                      <div style={s.previewActions}>
                        <button type="button" style={s.btnReEdit} onClick={() => rawFile && openEditor(rawFile)}>✏️ Re-edit</button>
                        <button type="button" style={s.btnRemove} onClick={removeImage}>🗑 Remove</button>
                      </div>
                    </div>
                  </div>
                ) : (
...
const s = {
  // Drop zone
  dropZone:      { border: '2px dashed #ccc', borderRadius: 8, padding: '1rem', textAlign: 'center', cursor: 'pointer', background: '#fafafa', minHeight: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8, transition: 'border-color .2s, background .2s' },
  ...
  thumbPreview:  { width: 80, height: 56, objectFit: 'cover', borderRadius: 6, border: '2px solid #4f46e5' },
  previewMeta:   { display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-start' },
  previewName:   { fontSize: 11, color: '#888', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  ...
  previewCanvas: { borderRadius: 8, border: '2px solid #4f46e5', display: 'block', maxWidth: '100%', userSelect: 'none' },
  // Dimension controls
  dimGrid:       { display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 10, alignItems: 'center', width: '100%' },
  dimBlock:      { display: 'flex', flexDirection: 'column', gap: 4 },
  dimLabelRow:   { display: 'flex', justifyContent: 'space-between' },
  dimLabel:      { fontSize: 12, fontWeight: 600, color: '#374151' },
  dimVal:        { fontSize: 12, color: '#6b7280' },
  slider:        { width: '100%', accentColor: '#4f46e5' },
  lockBtn:       { border: 'none', borderRadius: 6, width: 32, height: 32, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', alignSelf: 'flex-end', marginBottom: 2 },
  // Presets
  presetRow:     { display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center' },
  presetBtn:     { background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: 5, padding: '3px 10px', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#374151' },
